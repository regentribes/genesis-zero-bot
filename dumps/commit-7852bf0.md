=== COMMIT: 7852bf0 ===
7852bf0 Add Mythogen Framework glossary, Gherkin specs, and Ralph orchestrator prompts
...
docs/ecosystem-comparison-gaming.md
docs/mythogen/GLOSSARY.md
docs/mythogen/ORCHESTRATOR_PROMPTS.md
docs/mythogen/SPECIFICATIONS.feature
package-lock.json
package.json
radicle.toml
...
--- FILE: docs/ecosystem-comparison-gaming.md ---
# Game Engine & Graphics Framework Ecosystem Comparison

**Date:** 2026-03-29  
**Purpose:** Identify fit-for-purpose frameworks for AME tier 2/3 embedded deployment

---

## Overview

| Parameter | Bevy (Rust) | zig-gamedev |
|-----------|-------------|-------------|
| Stars | 45,331 ⭐ | 2,809 |
| Forks | 4,480 | 187 |
| Language | Rust | Zig |
| Paradigm | ECS (Entity-Component-System) | Composites + direct |
| Target | Desktop/embedded | Cross-platform |

### Alienkitty
- **alien.js** — JavaScript framework (not Rust/Zig relevant)
- **alienkitty** — Organization meta-repo
- **space.js**, **cyberspace** — Likely JavaScript projects
...
| Bevy | zig-gamedev |
|------|-------------|
| ECS with strict ownership | Manual memory, comptime arena |
| Borrow checker enforced | `arena.alloc()` patterns |
| Zero-cost abstractions | No hidden allocations |

### 2. Compile Time
| Bevy | zig-gamedev |
|------|-------------|
| Slow incremental compiles | Fast recompile |
| Heavy generics | Comptime metaprogramming |
| Procedural macros | `comptime {}` blocks |

### 3. Runtime Characteristics
| Bevy | zig-gamedev |
|------|-------------|
| ~45K stars, mature ecosystem | ~2.8K stars, growing |
| Plugin system | Direct integration |
| Official ECS + renderer | Graphics libs (zopengl, zd3d12) |

### 4. Bare Metal / Embedded Suitability

**Bevy advantages:**
- Strong type safety
- No garbage collector
- Excellent for Tier 1 (full stack)

**zig-gamedev advantages:**
- Smaller binary size
- No stdlib dependency (freestanding)
- Direct hardware access
- Excel for Tier 2/3 (bare metal ESP32/RISC-V)

### 5. Graphics Capabilities

| Feature | Bevy | zig-gamedev |
|---------|------|-------------|
| 2D rendering | ✅ Excellent | ✅ Via zopengl |
| 3D rendering | ✅ Excellent | ⚠️ Via zd3d12/wgpu |
| WebGPU | ✅ Built-in | Via zwebgpu |
| Shader system | WGSL | SPIRV-Vulkan |
| Asset pipeline | Built-in | DIY |
...
|------|-------------|-----------|
| Tier 1 (Pi 5) | **Bevy** | Full resources, rich ECS, mature tooling |
| Tier 2 (ESP32) | **zig-gamedev** | Small binary, freestanding, no GC |
| Tier 3 (RISC-V) | **zig-gamedev** | Power efficient, comptime, manual control |

### Decision Framework

- **Choose Bevy if:** Desktop-first, complex entity relationships, rapid development
- **Choose zig-gamedev if:** Embedded target, binary size constraints, power efficiency critical

---

## Recommendation

**Hybrid approach for AME:**
- Tier 1: Rust + Bevy for full FOT dashboard
- Tier 2/3: Zig + manual rendering for minimal footprint

This aligns with ADR-0001 (Rust + Zig stack) and enables:
- Shared logic (Four Distinctions, FOT calculations)
- Different rendering paths per tier
- Consistent data model across all tiers

---

## Additional Research Needed

- [ ] Evaluate bevy_ecs for embedded subset
- [ ] Test zig-gamedev on ESP32 target
- [ ] Benchmark binary sizes per tier
- [ ] Assess WebGPU for cross-tier rendering

--- FILE: docs/mythogen/GLOSSARY.md ---
...
--- FILE: docs/mythogen/ORCHESTRATOR_PROMPTS.md ---
...
--- FILE: docs/mythogen/SPECIFICATIONS.feature ---
