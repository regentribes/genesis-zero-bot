=== COMMIT: c0a976f ===
c0a976f Add project definition: Done criteria, objectives, outcomes
...
--- FILE: branches/seed-core/AffinityMapper.ts ---
/**
...
--- FILE: branches/seed-core/api.ts ---
/**
...
--- FILE: branches/seed-core/TrustDetector.ts ---
/**
...
--- FILE: branches/seed-core/types.ts ---
/**
...
# MYTHOGEN AME — Project Definition

## What Is This Project?

Mythogen AME (Affinity Mapping Engine) is a **regenerative community infrastructure** that distinguishes itself from extractive social platforms by treating technology as a silicon offspring — designed to foster mutual flourishing rather than harvest attention.

---

## Definition of Done

### Universal Done (All Stages)
- [ ] Gherkin acceptance tests pass (100% scenario coverage)
- [ ] Anti-capture mechanisms functional (Y-Cards, Time-Lock, Sacred Clown, Membrane, V-Crystal)
- [ ] No data extraction vectors (closed feedback loops)
- [ ] Transparent algorithm (WHY-Cards generated on every pattern detection)
...
| Stage | Done Criteria |
|-------|--------------|
| **seed-core** | Profile creation works; Four Distinctions separated; Time-Lock enforces 30-day maturation |
| **fot-tracker** | FOT calculates correctly (MIN, not average); 5 indicators tracked; status thresholds work |
| **values-flow** | LJMap loaded (130+ values); column distribution calculated; affinity scoring works |
| **anticapture** | Y-Cards render; Sacred Clown triggers; Membrane state machine functional |

### Per-Subsystem Done

| Subsystem | Done Criteria |
|-----------|---------------|
| **Living Seed Pattern** | Seed stores Need/Belief/Principle/Value separately; confidence scores update from behavior |
| **Trust Detector** | FOT = MIN(5 indicators); responds to trust events; emits FOT score |
| **Affinity Mapper** | Maps conversation to LJMap values; calculates column distribution |
| **Anti-Capture** | Time-Lock blocks matching <30 days; V-Crystal tracks shadow positions |
...
### Primary Objective
Build technology that generates **mutual flourishing** between community members, where the technology itself becomes a regenerative participant — not an extractive intermediary.

### Secondary Objectives
1. **Separate foundations rigorously** — Needs/Beliefs/Principles/Values must never conflate
2. **Detect Trust Field accurately** — Hologram principle (minimum, not average)
3. **Prevent extraction** — Anti-capture mechanisms always active
4. **Enable emergence** — Community coherence from bottom-up, not top-down design
...
- Seed core functions: profiles created with Four Distinctions separated
- FOT tracker calculates: single community, 5 indicators, MIN formula works
- Time-Lock enforces: 30-day maturation before matching allowed
- Y-Cards display: basic transparency on pattern detection
- **Outcome:** Functional prototype with 2-5 users, demonstrating core concepts

### Best-Case Scenario (Full Vision)
- Distributed mesh network: thousands of communities, each with autonomous FOT
- Concentrix orchestration: multiple simultaneous centers, no hierarchy
- Full LJMap integration: 130+ values across 3 cycles, 9 columns
- V-Crystal immune system: shadow positions tracked, cycle breaks prevented
- Living Membrane: semi-permeable boundaries, threats blocked, flourishing enabled
- Regeneration loop: community generates new seeds (Raw→Info→Conversation→Comprehension→Community→Regenerated)
- **Outcome:** Infrastructure standard for regenerative communities globally
...
### Stage 1: Seed Core
**What:** Core identity system for community members

**Done:**
- [ ] Profile stores Foundation (Need/Belief/Principle/Value)
- [ ] Matter state machine (Gas→Liquid→Solid→Plasma)
- [ ] Time-Lock (30 days)
- [ ] Confidence calculation from behavior

**Worst:** Single user, local storage only
**Best:** Millions of users, distributed identity

### Stage 2: FOT Tracker
**What:** Trust field detection using hologram principle

**Done:**
- [ ] Collect 5 indicators from community events
- [ ] Calculate FOT = MIN(5 indicators)
- [ ] Status thresholds (emerging/forming/established/thriving)
- [ ] Real-time updates via WebSocket

**Worst:** Single community, manual event entry
**Best:** Thousands of communities, automated event collection

### Stage 3: Values Flow
**What:** LJMap integration for value-based matching

**Done:**
- [ ] Load 130+ canonical values
- [ ] Map conversation to value statements
- [ ] Calculate column distribution (9 columns)
- [ ] Affinity scoring between profiles

**Worst:** Static value lists, simple matching
**Best:** Dynamic value evolution, emergent values

### Stage 4: Anti-Capture
**What:** Protection against extraction dynamics

**Done:**
- [ ] Y-Cards: Full algorithmic transparency
- [ ] Time-Lock: 30-day maturation
- [ ] Sacred Clown: Disruption to prevent closure
- [ ] Membrane: Semi-permeable boundary
- [ ] V-Crystal: Shadow position tracking

**Worst:** Basic time-lock only
**Best:** Full immune system for community dynamics
...
| Metric | Worst | Best |
|--------|-------|------|
| Communities | 1 | 10,000+ |
| Members per community | 5 | 500 |
| FOT accuracy | Manual | Automatic |
| Anti-capture | Time-lock only | Full suite |
| Values mapped | 10 | 130+ |
...
```
coherence = column_dist×0.35 + fot×0.25 + gate×0.20 + domain×0.20
```
